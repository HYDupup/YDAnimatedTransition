...
@implementation LevelLineTransition
...
-(NSMutableArray *)creatSnapshots:(UIView *)view afterScreenUpdates:(BOOL)afterScreenUpdates{
    ...
    CGFloat width = 4.0;
    int number = (int)(view.width/width);
    for (int i=0; i <= number; i++) {
        CGRect frame = CGRectMake(i*width, 0, width, view.height);
...
-(void)removeSnapshots:(NSArray<UIView *> *)snapshots{
    ...
    for (UIView *snapshotView in snapshots) {
        ...
        CGFloat offsetY = CGRectGetHeight(snapshotView.frame) * RANDOM_FLOAT(1.0, 4.0);
        snapshotView.ty = direction ? -offsetY : offsetY;
        direction = !direction;
